...
if __name__ == '__main__':
    with requests.Session() as session:
        ...
        ordinal_check_date = ordinal_check_date = datetime.date.today().toordinal()
        blacklist = []
        tag_groups = []
            ...
            if section.lower() == 'settings':
                for option, value in config.items(section):
                    if option == 'days_to_check':
                        ordinal_check_date = datetime.date.today().toordinal() - int((config.get('Settings', 'days_to_check'))) + 1
        ...
        for group in tag_groups:
            ...
            for i in count(start = 1):
                results = remote.get_posts(search_string, min_score, check_date, i, constants.MAX_RESULTS, session)
